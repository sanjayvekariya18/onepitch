<h2 class="text-center">
    @if(Auth::check() && Auth::user()->role == App\Models\User::ROLE_JOURNALIST)
        What topics do you cover under
    @elseif(Auth::check() && Auth::user()->role == App\Models\User::ROLE_PUBLICIST)
        What topics does your client, or brand, cover under
    @endif
    {{ $industry->title }}?</h2>
...
<div class="text-center">
    <a href="javascript:;"
       id="add-topics"
       onclick="common.addIndustryTopics('{{ $industry->title }}', '{{ $industry->id }}');"
       class="btn btn-white-ylw btn-submit m-b-64" disabled>Next</a>
</div>

<script>
	$(function () {
        common.bindTopicsFromTag('#industry', '{{ $industry->id }}');
	})
</script>
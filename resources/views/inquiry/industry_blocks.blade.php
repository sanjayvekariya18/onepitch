@if (count($industries['items']))
    @foreach ($industries['items'] as $industry)
        <div id="industry-block{{ $industry->id }}" class="col-sm-6 col-md-4 industry {{isset($selected[$industry->id]) ? 'checked' : ''}}"
             onclick="inquiry.selectIndustry('{{ $industry->title }}', '{{ $industry->id }}');">
...
<script>
    var landedIndustry = {{!empty($landedIndustryId) ? $landedIndustryId : 0}}
    ...
    @endif;
    var ids = $(".tag").map(function() {
        return parseInt(this.id.substr(8));
    }).get();

    industries = {!! $industries['items'] !!};
    industries.map(function (industry) {
        //console.log($.inArray(industry.id, ids));
        if ($.inArray(industry.id, ids) > 0) {
            var industryBlock = $('#industry-block' + industry.id)
            if(!industryBlock.hasClass("checked")){
                industryBlock.addClass("checked");
                $(industryBlock.find('.check-mark')[1]).css('opacity',1);
            }
        }
    });

    if (landedIndustry) {
        $('#industry-block' + landedIndustry).trigger("click");
    }
</script>
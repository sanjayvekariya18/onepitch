            ...
            <div class="table-wrapper with-header">
                <table class="table striped highlight clickable">
                    ...
                    <tr v-for="user in users" v-on:click="showProfile(user)">
                        ...
                        <td class="buttons">
                            <button v-on:click.stop="denyUserModal(user)" class="btn btn-white btn-large btn-width-96">DENY</button>
                            <button v-on:click.stop="approveUserModal(user)" class="btn btn-yellow btn-large btn-width-96">APPROVE</button>
                        </td>
                    </tr>
                    </tbody>
                </table>
                <div class="no-new-journalist-result" v-if="users.length < 1">Sorry, no result found...</div>
            </div>
...
<script type="text/babel">
    ...
    export default {
        ...
        methods : {
            getUsers: function () {
                this.loading = true;
                this.$http.get(laroute.route('admin.rest.journalists.new', {sort: 'desc'}))
                    .then(response => {
            ...
            denyUserModal : function (user) {
                this.$refs.denyJournalistModal.open(user);
            },

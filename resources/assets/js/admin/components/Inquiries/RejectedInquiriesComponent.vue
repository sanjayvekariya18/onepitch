<template>
    <div class="list-items inquiries rejected" :class="{spinner: loading}">
        <preloader v-if="loading"></preloader>
        <div v-show="!loading">
            <div class="table-header">
                <div class="row">
                    <div class="col title">Rejected Inquiries - {{ inquiries.length }}</div>
              ...
              <div class="col s12">
                <datatable
                    ...
                    <template slot="tbody-tr" scope="props">
                        ...
                        <td class="date">{{ props.row['uploadedFromNow'] }}</td>
                    </template> 
                </datatable>
              </div>
            </div>  
    ...
    export default {
        ...
        methods : {
            getInquiries: function () {
                this.loading = true;
                this.$http.get(laroute.route('admin.rest.inquiries.rejected', {sort: 'desc'}))
                    .then(response => {
                        var formatted = [];
                        let i;
                        for (let i = 0; i < response.data.length; i++) {
                            formatted[i] = {
                                ...
                                sent_at: response.data[i].sent_at                           
                            };
                        }                         
                        this.rows = formatted;

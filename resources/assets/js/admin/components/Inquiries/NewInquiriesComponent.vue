<template>
    <div class="list-items inquiries inquiries-new" :class="{spinner: loading}">
        <preloader v-if="loading"></preloader>
        <div v-show="!loading">
            <div class="table-header">
                <div class="row">
                    <div class="col title">Needs review - {{ inquiries.length }}</div>
                    <div class="col sorter right-align">
                        <date-range-component v-bind:pitches="inquiries" @filtered-by-date="filteredInquiries"></date-range-component>
                    </div>
                </div>
            </div>
            <div class="row">
              <div class="col s12">
                <datatable
                    ...
                    <template slot="tbody-tr" scope="props">
                        ...
                        <td class="button"><button class="btn btn-yellow btn-large btn-width-160" @click="showInquiry(props.row)">REVIEW</button></td>
                    </template> 
                </datatable>
              </div>
            </div>
    ...
    export default {
        ...
        methods : {
            getInquiries: function () {
                this.loading = true;
                this.$http.get(laroute.route('admin.rest.inquiries.new', {sort: 'desc'}))
                    .then(response => {
            ...
            showInquiry : function (row) {
                this.$router.push({ name: 'inquiries.show', params: { inquiryId: row.id }});
            },

<template>
    <div :class="{spinner: loading}">
        ...
        <div v-show="!loading">
            ...
            <div class="row">
                <div class="col s12">
                    <datatable title="Journalists (By Topic)" :columns="columns" :rows="rows"
...
<script type="text/babel">
  ...
  export default {
    name: 'JournalistTopicSubscription',
    components: {
      Preloader,
      LineChart,
      BarChart,
    ...
    data () {
      return {
        ...
        columns: [
          ...
          { id: 5, label: 'Topic', field: 'topic', numeric: false, html: false, width: 'auto' }
        ],
      }
    },
    methods: {
      ...
      getData: function () {
        this.loading = true
        this.$http.get(laroute.route('admin.reports.journalists.topic-subscription'))
          .then(response => {
            let rows = response.data.journalistTopicSubscriptions
            this.rows = rows
            this.loading = false
            ...
            rows.forEach(function (value) {
              if (labels.indexOf(value.topic) === -1) {
                labels.push(value.topic)
              }
            })

            labels.forEach(function (e) {
              let o = Math.round, r = Math.random, s = 255
              let subscriptionByTopic = rows.filter(r => r.topic == e).length
              ...
              dataAvg.push(parseFloat(subscriptionByTopic * 100 / totalSubscriptions).toFixed(2))
            })
      ...
      fillLineChart (dataGraph) {
        this.titleGraph = 'Journalists by Topic'
        this.datacollection = {

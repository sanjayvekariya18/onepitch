    ...
    <div :class="{spinner: loading}">
        ...
        <div v-show="!loading">
            ...
            <div class="row">
                ...
                <div class="col s12">
                    <datatable title="Saved Inquiry" :columns="columns" :rows="rows"
  ...
  export default {
    ...
    data () {
      return {
        ...
        columns: [
          ...
          { id: 2, label: 'Inquiry ID', field: 'inquiry_id', numeric: true, html: false, width: 'auto' },
          { id: 3, label: 'User ID', field: 'user_id', numeric: true, html: false, width: 'auto' },
          { id: 4, label: 'Publicist Name', field: 'full_name', numeric: false, html: false, width: 'auto' },
    ...
    methods: {
      ...
      getData: function (filters) {
        this.loading = true
        this.$http.get(laroute.route('admin.reports.publicists.saved-inquiries', filters))
          .then(response => {
            let savedInquiries = response.data.savedInquiries
            this.rows = savedInquiries
            this.loading = false
            ...
            savedInquiries.forEach(function (value) {
              if (dataGraph.users.indexOf(value.user_id) == -1) {
                dataGraph.users.push(value.user_id)
              }
              if (dataGraph.inquiries.indexOf(value.inquiry_id) == -1) {
                dataGraph.inquiries.push(value.inquiry_id)
              }
            })
          ...
          datasets: [
            {
              ...
              lineTension: 0.1,
              fill: false,
              data: [dataGraph.users.length, dataGraph.inquiries.length]
            },
          ]
        }
      },

SELECT i.title                                                                                  AS industry,
       ...
       (SELECT count(*) FROM pitch_mail_statistics WHERE pitch_mail_statistics.pitch_id = p.id) AS number_of_responses,
       ...
       (SELECT count(*) FROM pitch_industry WHERE pitch_industry.pitch_id = p.id)               AS number_of_industries,
       ...
       wordcount(CONCAT(p.`why_point_1`, ' ', p.`why_point_2`, ' ', p.`why_point_3`))           AS word_count_why,
       IF(p.website IS NOT NULL, 'yes', 'no')                                                   AS hyperlink,
       IF((SELECT COUNT(*) FROM pitch_press_releases WHERE pitch_id = p.id), 'yes', 'no')       AS press_release,
       IF((SELECT COUNT(*) FROM pitch_files WHERE pitch_id = p.id), 'yes', 'no')                AS media_attachments
FROM pitches p
